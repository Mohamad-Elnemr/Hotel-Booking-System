...
const RoomDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        response.room.processedImg = 'data:image/jpeg;base64,' + response.room.byteImg
  ...
  const { roomType, roomPrice , processedImg, description, bookings } = roomDetails;
  //const roomPhoto = rooms[0].processedImg ;
  ...
  return (
    <div className="room-details-booking">
      ...
      <h2>Room Details</h2>
      <br />
      <img src={ processedImg } alt={roomType} className="room-details-image" />
        ...
        {showDatePicker && (
          <div className="date-picker-container">
            <input 
              ...
              value={checkInDate}  // set as a controlled input
              onChange={(e) => setCheckInDate(e.target.value)}  // handle date selection
              min={checkInDate}  // sets the minimum selectable date (optional)
              max={checkOutDate}  // sets the maximum selectable date (optional)
            ...
            <input
              type="date"
              className="detail-search-field"
              value={checkOutDate}  // set as controlled input
              onChange={(e) => setCheckOutDate(e.target.value)}  // update state when date changes
              min={checkInDate}  // restricts selection to dates after check-in date (optional)
